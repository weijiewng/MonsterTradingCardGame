    ...
    public void save(Card card){
        try(Connection connection = getConnection();
            PreparedStatement statement = connection.prepareStatement(
                    "INSERT INTO card (id, class, name, damage, element, rarity, type) VALUES (?, ?);"
            )
        ){
            statement.setString(1, card.getId());
            statement.setString(2, card.getClass().getName());
            statement.setString(3, card.getName());
            statement.setInt(4, card.getDamage());
            statement.setString(5, card.getElement().toString());
            statement.setString(6, card.getRarity().toString());
            if(card instanceof MonsterCard){
                statement.setString(7, ((MonsterCard) card).getType().toString());
            }
            statement.execute();
        }
